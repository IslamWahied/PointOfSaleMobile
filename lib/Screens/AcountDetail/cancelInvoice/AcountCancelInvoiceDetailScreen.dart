...
class AcountCancelInvoiceDetailScreen extends StatelessWidget {
  String selectedIcon;
  String heroTag;
  String titleName;
  int typeId;

  AcountCancelInvoiceDetailScreen({Key key, this.selectedIcon, this.heroTag, this.titleName,this.typeId})
      : super(key: key);
                  ...
                  Container(
                    ...
                    child: Column(
                      children: <Widget>[
                        ...
                        (cubit.list.where((element) => convertStringToDateTime(element.EntryDate).day == DateTime.now().day && convertStringToDateTime(element.EntryDate).month ==DateTime.now().month && convertStringToDateTime(element.EntryDate).year == DateTime.now().year).toList().isNotEmpty && cubit.isShowTodayAccountOnly) ||
                        ...
                        ListView.separated(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              if(cubit.isShowTodayAccountOnly){
                                model = cubit.list.where((element) => convertStringToDateTime(element.EntryDate).day == DateTime.now().day && convertStringToDateTime(element.EntryDate).month ==DateTime.now().month && convertStringToDateTime(element.EntryDate).year == DateTime.now().year).toList()[index];
                              }
                              else{
                              ...
                              return Material(
                                ...
                                child: ListTile(
                                  ...
                                  trailing:     Text("+\$ ${model.FinalTotal}",
                                      style: const TextStyle(
                                          fontSize: 18,
                            ...
                            itemCount: cubit.isShowTodayAccountOnly?  cubit.list.where((element) => convertStringToDateTime(element.EntryDate).day == DateTime.now().day && convertStringToDateTime(element.EntryDate).month ==DateTime.now().month && convertStringToDateTime(element.EntryDate).year == DateTime.now().year).toList().length:cubit.list.length
