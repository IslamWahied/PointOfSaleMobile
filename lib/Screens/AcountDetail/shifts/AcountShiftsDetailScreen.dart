...
class AcountShiftsDetailScreen extends StatelessWidget {
  String selectedIcon;
  String heroTag;
  String titleName;
  int typeId;

  AcountShiftsDetailScreen({Key key, this.selectedIcon, this.heroTag, this.titleName,this.typeId})
      : super(key: key);
                  ...
                  Container(
                    ...
                    child: Column(
                      children: <Widget>[
                        ...
                        (cubit.list.where((element) => convertStringToDateTime(element.shiftEndDate).day == DateTime.now().day && convertStringToDateTime(element.shiftEndDate).month ==DateTime.now().month && convertStringToDateTime(element.shiftEndDate).year == DateTime.now().year  ).toList().isNotEmpty && cubit.isShowTodayAccountOnly) ||
                        ...
                        ListView.separated(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              if(cubit.isShowTodayAccountOnly){
                                model = cubit.list.where((element) => convertStringToDateTime(element.shiftEndDate).day == DateTime.now().day && convertStringToDateTime(element.shiftEndDate).month ==DateTime.now().month && convertStringToDateTime(element.shiftEndDate).year == DateTime.now().year ).toList()[index];
                              }
                              else{
                              ...
                              return Material(
                                ...
                                child: ListTile(
                                  ...
                                  subtitle:Text(intl.DateFormat('yyyy-MM-dd\nhh:mm a').format(convertStringToDateTime(model.shiftEndDate))),

                                  trailing:      Text((model.shiftEndAmount + model.expenses) > 0 ? "+\$ ${ model.shiftEndAmount + model.expenses }":"-\$ ${ model.shiftEndAmount + model.expenses }",
                                      style:   TextStyle(
                                          fontSize: 18,
                                          color: model.shiftEndAmount > 0 ? Colors.green :Colors.redAccent)),
                                  leading: CircleAvatar(
                                    radius: 25,
                                    child: Image.asset(

                                      model.shiftEndAmount > 0 ?  "assets/images/revenue.png" : IconImgs.transfer  ,
                                      height: 30,
                                      width: 30,
                                    ),
                                    backgroundColor:
                                    model.shiftEndAmount > 0 ?   Colors.green[100] : IconColors.transfer
                                    ,
                                  ),
                                  enabled: true,
                                  onTap: () {},
                            ...
                            itemCount: cubit.isShowTodayAccountOnly?  cubit.list.where((element) => convertStringToDateTime(element.shiftEndDate).day == DateTime.now().day && convertStringToDateTime(element.shiftEndDate).month ==DateTime.now().month && convertStringToDateTime(element.shiftEndDate).year == DateTime.now().year  ).toList().length:cubit.list.toList().length
